...
```{r}
#| label: ejercicio_23
# Gráfico de densidad con escala original
p3 <- ggplot(gapminder, aes(x = gdpPercap, fill = continent)) +
  ...
  labs(
    ...
    subtitle = "Escala original del PIB",
    x = "PIB per cápita (USD)",
    y = "Densidad",
    fill = "Continente"
  ) +
...
# Gráfico de densidad con escala logarítmica
p4 <- ggplot(gapminder, aes(x = gdpPercap, fill = continent)) +
  geom_density(alpha = 0.5) +
  scale_x_log10(labels = scales::dollar_format()) +
  facet_wrap(~year, nrow = 4, ncol = 3) +
  labs(
    title = "Distribución del PIB per cápita por año",
    subtitle = "Escala logarítmica del PIB",
    x = "PIB per cápita (USD, escala log)",
    y = "Densidad",
    fill = "Continente"
  ) +
